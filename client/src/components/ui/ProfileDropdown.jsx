...
const ProfileDropdown = ({ logout, onLanguageChange }) => {
  const { user } = useAuth();
  ...
  const getProfileImageUrl = () => {
    ...
    const profileImagePath = user?.profileImage || user?.profilePhoto;
  ...
  const MenuContent = () => (
    <>
      {/* User Info Header */}
      <div
        className={`${isMobile ? "px-6 py-4" : "px-4 py-3"} border-b border-gray-100`}
      >
        <div className="flex items-center">
          ...
          <div className="ml-3 flex-1">
            <p
              className={`${isMobile ? "text-base" : "text-sm"} font-medium text-gray-700`}
            >
              {displayName}
            </p>
            ...
            <p
              className={`${isMobile ? "text-sm" : "text-xs"} mt-1 ${roleConfig.roleColor}`}
            >
      ...
      <div className={`${isMobile ? "py-2" : "py-1"}`}>
        ...
        >
          <ChartBarIcon
            className={`${isMobile ? "h-6 w-6" : "h-4 w-4"} mr-3`}
          />
