...
const CandidateCard = ({
  ...
  const getCurrentStatus = () => {
    ...
    if (
      ...
      const selectedAllocation = candidate.allocations.find(
        (allocation) => allocation.id === selectedJobId,
      );
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    let allocation = candidate.allocations.find(
      (alloc) => alloc.id === selectedJobId,
    );
    if (!allocation) {
  ...
  const handleJobStatusUpdate = async (allocationId, newStatus) => {
    ...
    try {
      ...
      showError("Failed to update job application status");
    } finally {
  ...
  const handleViewProfile = () => {
    ...
    if (candidateId) {
      ...
      showError("Unable to view profile - candidate ID not found");
    }
  };
  ...
  const handleBookmarkToggle = async () => {
    ...
    try {
      const success = await onBookmarkToggle(candidate.id, !isBookmarked);
      if (success) {
  ...
  return (
    <>
      <div
        className={`bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 border border-gray-200 hover:border-blue-300 w-full ${className}`}
      >
                ...
                <div className="flex items-start space-x-3 lg:space-x-4 mb-2 lg:mb-3">
                  <img
                    ...
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${candidate.user?.name}&background=1976d2&color=fff`
                    }
                    ...
                    {jobApplications.map((application) => (
                      ...
                      >
                        <span className="font-medium text-gray-900 truncate flex-1 mr-1 lg:mr-2">
                          ...
                          {selectedJobId === application.id && (
                            <span className="ml-1 text-xs lg:text-sm text-blue-600 font-medium lg:inline hidden">
                              (Selected)
                            </span>
                          )}
                        </span>
                        <div className="flex items-center space-x-1 lg:space-x-2 flex-shrink-0">
                          {editingJobStatus === application.id ? (
                            ...
                            <>
                              <span
                                className={`px-1.5 py-0.5 lg:px-2 lg:py-1 text-xs lg:text-sm font-medium rounded border ${getStatusBadgeColor(application.status)}`}
                              >
            ...
            <div className="p-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  ...
                  className="flex items-center justify-center p-2.5 rounded-lg bg-orange-500 hover:bg-orange-600 text-white shadow-sm active:scale-95 transition-all duration-150"
                ...
                <Button
                  ...
                  className="flex items-center justify-center p-2.5 rounded-lg bg-gray-600 hover:bg-gray-700 text-white shadow-sm active:scale-95 transition-all duration-150"
                  title="View profile"
                >
                ...
                <Button
                  ...
                  className={`flex items-center justify-center p-2.5 rounded-lg shadow-sm active:scale-95 transition-all duration-150 ${
                    isBookmarked
                      ? "bg-red-500 hover:bg-red-600 text-white"
                      : "bg-pink-500 hover:bg-pink-600 text-white"
                  }`}
                  title={
                    isBookmarked ? "Remove bookmark" : "Bookmark candidate"
                  }
                >
                  {isBookmarked ? (
              ...
              {actionButtons.length > 0 && (
                <div className="grid grid-cols-2 gap-1">
                  {actionButtons.map((actionData, index) => {
                    ...
                    const isLoading =
                      loading[actionData.action] ||
                      internalLoading[actionData.action];
                    const currentStatus = getCurrentStatus();
                  ...
                  <div className="flex space-x-2">
                    {actionButtons.slice(3).map((actionData, index) => {
                      ...
                      const isCurrentStatus =
                        actionData.action === currentStatus;
