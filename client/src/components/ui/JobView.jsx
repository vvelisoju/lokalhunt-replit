          ...
          <div className="space-y-3 sm:flex sm:space-y-0 sm:space-x-3 sm:justify-end">
            {user?.role === "CANDIDATE" && (
              <>
                ...
                {/* Call button for candidates who have applied */}
                {hasApplied && job.employer?.user?.phone && (
                  <Button
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    variant="outline"
                    className="w-full sm:w-auto flex items-center justify-center py-3 sm:py-2 text-sm font-medium transition-all duration-200 text-green-600 hover:text-green-700 border-green-200 hover:border-green-300"
                  >
                    <svg className="h-4 w-4 sm:h-5 sm:w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
